...
const SelectAssetsInput = ({
  ...
  const handleSetAmount = (el: number) => {
    ...
    const amount = new BN(
      BN.parseUnits(new BN(selectedOption.balance ?? 0).multipliedBy(el).div(new BN(100)), decimals).decimalPlaces(
        0,
        BN.ROUND_DOWN,
      ),
    );
  ...
  const isInputError = false; //todo: check if input is error
  return (
